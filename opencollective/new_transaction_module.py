...
csvFile_main = open('Budget&team_details.csv', 'w')
writer_main = csv.writer(csvFile_main)
writer_main.writerow(['Project Name', 'Current Budget', 'Team Info'])
...
for instance, alpha in enumerate(all_url):
	...
	team_list = ""
	if (len(amt) > 0):
		team_list = ((amt[2].text).encode('utf-8'))
		amt_list = ((amt[3].text).encode('utf-8'))
	writer_main.writerow([all_names[instance], amt_list, team_list])
